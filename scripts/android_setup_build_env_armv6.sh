...
OPENSSL_SCM=https://github.com/bmiklautz/android-external-openssl-ndk-static.git
...
#SSL_ROOT=`
rm -r $ROOT/openssl
ln -s $OPENSSL_SRC $ROOT/openssl
#ln -s $SSL_ROOT $ROOT/openssl
#mkdir -p $ROOT/openssl/obj/local/armeabi/
#cp $ROOT/openssl-build/libssl.a $ROOT/openssl/obj/local/armeabi/
...
cd $NDK_PROFILER_SRC
ndk-build V=1 APP_ABI=armeabi clean
ndk-build V=1 APP_ABI=armeabi
RETVAL=$?
if [ $RETVAL -ne 0 ]; then
	echo "Failed to execute ndk-build command [$RETVAL]"
	exit -6
